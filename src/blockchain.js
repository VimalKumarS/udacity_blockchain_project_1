...
class Blockchain {
    ...
    requestMessageOwnershipVerification(address) {
        return new Promise((resolve) => {
            ...
            const dt = new Date().getTime().toString().slice(0, -3);
            const message_ownership = `${address}:${dt}:starRegistry`;
            resolve(message_ownership);
        });
    }
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            const block_data = { address: address, message: message, signature: signature, star: star };
            const block = new BlockClass.Block(block_data);
            await this._addBlock(block);
            resolve(block);
        });
    }
    ...
    getStarsByWalletAddress(address) {
        ...
        return new Promise((resolve, _reject) => {
            self.chain.forEach(async (block) => {
                ...
                if (block_data!==null && block_data.address === address) stars.push(block_data);
            });
            resolve(stars);
        });
    }
